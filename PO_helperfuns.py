...
def NextGen(uvec,xvec,rvec,K,pc,beta,deltas = [0, 0], eta=1):
    ...
    if np.array(p1).size==1: # p1 is a scalar
        ...
        denom_r = r1 + r2
        R_1 = r1/denom_r if denom_r>0 else 0
        R_2 = r2/denom_r if denom_r>0 else 0
    else:
        ...
        mask_r1r2pos = r1 + r2 > 0
        denom = r1[mask_r1r2pos] + r2[mask_r1r2pos]
        
        R_1 = np.zeros(np.array(r1).shape)
        R_1[mask_r1r2pos] = r1[mask_r1r2pos]/denom
        
        
        R_2 = np.zeros(np.array(r2).shape)
        R_2[mask_r1r2pos] = 1 - R_1[mask_r1r2pos]
                     
    

    Wu1 = u*(1+r1)*(K*psi_1 + pc*R_1)
    Wu2 = u*(1+r2)*(K*psi_2 + pc*R_2)
    Wbu = u*(1-pc-K)
    
    Wx1 = x*(1+r1)*(K_x*psi_1 + pc_x*R_1)
    Wx2 = x*(1+r2)*(K_x*psi_2 + pc_x*R_2)
    Wbx = x*(1-pc_x-K_x)

    
    W = Wu1 + Wu2 + Wbu + Wx1 + Wx2 + Wbx 
    def get_freq(Wui, W):
        if np.array(p1).size==1:
            to_return = Wui/W if W>0 else 0
            return(to_return)
        else:
            ans = np.zeros(np.array(Wui).shape)
            mask = W > 0
            ans[mask] = Wui[mask]/W[mask]
            return(ans)
        
    freqs = [get_freq(item,W) for item in [Wu1, Wu2, Wbu, Wx1, Wx2, Wbx]]
