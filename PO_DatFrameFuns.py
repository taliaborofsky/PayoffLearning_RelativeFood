...
def GetXsteps(param_grid, tsteps):
    ...
    # want to do sqrt((u1_2 - u1_1)^2 + (u2_2 - u2_1)^2 + ...)
    # want to take sum of square differences entrywise over u1, u2, bu, ... vectors
    dist = np.sqrt(np.sum((curr - nextval)**2,axis=0)) 
    reached_eq = dist < 1e-10
